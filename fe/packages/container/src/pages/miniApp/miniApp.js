...
export class MiniApp {
	...
	showModal(opts = {}) {
		...
		// 一次解析 callback，闭包到 cfg 里，避免 funcId 失效或重复创建
		const onSuccess = this.createCallbackFunction(success)
		const onFail = this.createCallbackFunction(fail)
		const onComplete = this.createCallbackFunction(complete)
		...
		this._modalStack.push(cfg)

		// === debug ===
		console.log(
			`[showModal] push title=${JSON.stringify(title)} content=${JSON.stringify(content)} ` +
			`current=${this._currentCfg ? JSON.stringify(this._currentCfg.title || this._currentCfg.content) : 'null'} ` +
			`stack=[${this._modalStack.map(c => JSON.stringify(c.title || c.content)).join(', ')}]`,
		)

		// 抢占式 LIFO：新 push 的若不是当前正在显示的，就把当前 dialog 撤掉换上栈顶
		this._processModalQueue()
	}

	/**
	 * 检查栈顶并同步显示。每次 push / userDismiss / 抢占动画结束后调用。
	 * - 栈空 + 无当前：no-op
	 * - 有栈顶 + 无当前：立即渲染（首次显示无延迟）
	 * - 栈顶 !== 当前：100ms 节流（leading-edge）后抢占。
	 *   节流窗内的连续 push 都不会触发新的定时器，等定时器到点时读最新栈顶来切换 ——
	 *   这样 onLoad/onShow/onReady 这种紧密连续的 push 只发生一次可见的 dialog 切换。
	 */
	_processModalQueue() {
		if (this._dismissTransitionPending) {
			// 转场动画进行中，等动画结束后会自动再调一次本方法
			return
		}

		const top = this._modalStack && this._modalStack.length > 0
			? this._modalStack[this._modalStack.length - 1]
			: null

		if (!top && !this._currentCfg) {
			return
		}
		...
		if (top !== this._currentCfg) {
			// 抢占：100ms 节流
			this._schedulePreemptSwap()
		}
		// else: 栈顶就是当前显示的 cfg，无需变更
	}

	/**
	 * 100ms 节流（leading-edge）：第一次 push 触发抢占时设置定时器，
	 * 节流窗内的后续 push 不重设定时器（栈持续被填充）；
	 * 定时器到点时读最新栈顶执行切换。
	 */
	_schedulePreemptSwap() {
		if (this._preemptTimer) return
		this._preemptTimer = setTimeout(() => {
			this._preemptTimer = null
			if (this._dismissTransitionPending) return
			const top = this._modalStack.length > 0
				? this._modalStack[this._modalStack.length - 1]
				: null
			if (!top || top === this._currentCfg) return
			console.log(
				`[showModal] preempt → swap from ${JSON.stringify(this._currentCfg?.title || this._currentCfg?.content)} ` +
				`to ${JSON.stringify(top.title || top.content)}`,
			)
			this._dismissCurrent(false, () => this._renderTopModal())
		}, 100)
	}

	/**
	 * 用户主动关闭（点按钮或遮罩）。把 cfg 从栈中移除并触发回调，再处理队列。
	 */
	_userDismissModal(confirmed) {
		const cfg = this._currentCfg
		if (!cfg) return

		// 从栈中移除（用户已亲手关闭）
		const idx = this._modalStack.indexOf(cfg)
		if (idx >= 0) {
			this._modalStack.splice(idx, 1)
		}

		this._dismissCurrent(true, () => {
			cfg.onSuccess?.({
				confirm: confirmed,
				cancel: !confirmed,
				errMsg: 'showModal:ok',
	...
	/**
	 * 撤掉当前 dialog；如果 fadeMaskIfEmpty=true 且栈空则同时淡出 mask。
	 * @param {boolean} fadeMaskIfEmpty 仅 user dismiss 路径会传 true（抢占场景下 mask 永远保留）
	 * @param {Function} afterCallback 动画结束后执行
	 */
	_dismissCurrent(fadeMaskIfEmpty, afterCallback) {
		...
		this._dismissTransitionPending = true
		this._currentDialog = null
		this._currentCfg = null
		this._currentMaskHandler = null

		dialog.classList.remove('show')

		const fadeMask = fadeMaskIfEmpty && this._modalStack.length === 0 && this._modalMask
		if (fadeMask) {
			this._modalMask.classList.remove('show')
		}
		...
		setTimeout(() => {
			...
			this._dismissTransitionPending = false
			afterCallback?.()
		}, 200)
	}

	/**
	 * 渲染栈顶 cfg 的 dialog。复用 mask（首次创建时绑定永久 click 委托）。
	 * 不会移除栈中元素；元素的"消费"由 _userDismissModal 在用户操作时完成。
	 */
	_renderTopModal() {
		const cfg = this._modalStack && this._modalStack.length > 0
			? this._modalStack[this._modalStack.length - 1]
			: null
		if (!cfg) return

		console.log(
			`[showModal] render title=${JSON.stringify(cfg.title)} content=${JSON.stringify(cfg.content)} ` +
			`stack-depth=${this._modalStack.length}`,
		)

		// Mask：复用或新建
		let mask = this._modalMask
		let isMaskNew = false
		if (!mask) {
			mask = document.createElement('div')
			mask.className = 'dimina-dialog-mask'
			// 永久 click 委托：转发给当前 dialog 的 handler。每次渲染时更新 _currentMaskHandler
			mask.addEventListener('click', () => {
				this._currentMaskHandler?.()
			})
			this.webviewsContainer.appendChild(mask)
			this._modalMask = mask
			isMaskNew = true
		}
		...
		if (cfg.showCancel) {
			...
			cancelBtn.style.color = cfg.cancelColor
		...
		confirmBtn.style.color = cfg.confirmColor
		buttonRow.appendChild(confirmBtn)

		dialog.appendChild(buttonRow)

		// 当前 cfg / dialog / 委托 handler 引用
		this._currentDialog = dialog
		this._currentCfg = cfg
		// 微信规范：showCancel:false 时点击遮罩不关闭，强制用户操作 confirm 按钮
		this._currentMaskHandler = cfg.showCancel ? () => this._userDismissModal(false) : null

		cancelBtn?.addEventListener('click', () => this._userDismissModal(false))
		confirmBtn.addEventListener('click', () => this._userDismissModal(true))

		this.webviewsContainer.appendChild(dialog)

		// 触发渐入：等浏览器把初始态绘一帧后再加 .show
		requestAnimationFrame(() => {
			// 仅在 mask 是新建时才触发淡入；后续抢占切换中 mask 保持 .show 不变
			if (isMaskNew) {
